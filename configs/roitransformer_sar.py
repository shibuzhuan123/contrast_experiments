# RoI Transformer 配置文件 - SAR 旋转目标检测
# 基于 faster_rcnn_RoITrans_r50_fpn_1x_dota.py 修改
...
model = dict(
    ...
    bbox_head=dict(
        ...
        roi_feat_size=7,
        num_classes=6,  # SAR数据集: ship, aircraft, car, tank, bridge, harbor
...
data_root = 'data/dota_sar/'  # 修改为SAR数据集路径
img_norm_cfg = dict(
...
checkpoint_config = dict(interval=1)  # 每个epoch保存一次
...
log_level = 'INFO'
work_dir = './work_dirs/roitransformer_sar'
load_from = None
resume_from = None
workflow = [('train', 1)]
